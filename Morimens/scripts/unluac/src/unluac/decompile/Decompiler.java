...
public class Decompiler {
  ...
  private void handle54BinKOp(List<Operation> operations, State state, int line, Expression.BinaryOperation op) {
    // If the expected MMBINK is missing, skip instead of crashing
    if(line + 1 > code.length || code.op(line + 1) != Op.MMBINK) {
      return;
    }
  ...
  private List<Operation> processLine(State state, int line) {
    ...
    switch(code.op(line)) {
        ...
        case ADDI: {
          ...
          if(line + 1 > code.length || code.op(line + 1) != Op.MMBINI) {
            ...
            operations.add(new RegisterSet(line, A,
                Expression.make(Expression.BinaryOperation.ADD, left, right)));
            break;
          }
      
          // ✅ Existing MMBINI paired logic below
          Expression.BinaryOperation op = decodeBinOp(code.C(line + 1));
          int immediate = code.sC(line);
          boolean swap = false;
          if(code.k(line + 1)) {
            if(op != Expression.BinaryOperation.ADD) {
              // ✅ Not valid for metamethod — skip instead of throwing
              break;
            }
            swap = true;
          } else {
            if(op == Expression.BinaryOperation.ADD) {
              ...
              // ✅ Don't kill the decompiler — just skip
              break;
            }
          }
      ...
      case BXORK:
        ...
        break;
      case SHRI: {
        if(line + 1 > code.length || code.op(line + 1) != Op.MMBINI) break;//throw new IllegalStateException();
        int immediate = code.sC(line);
        Expression.BinaryOperation op = decodeBinOp(code.C(line + 1));
