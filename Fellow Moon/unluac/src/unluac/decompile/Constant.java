    ...
    switch(type) {
      ...
      case 3:
        ...
        for(int i = 0; i < string.length(); i++) {
          ...
          if(c == '\n') {
            newlines++;
          } else if(c <= 31 && c != '\t') {  // FIXED: Removed c >= 127 check
            unprintable++;
          }
        }
        if(unprintable == 0 /* && !string.contains("[[") */ && (newlines > 1 || (newlines == 1 && string.indexOf('\n') != string.length() - 1))) {
          ...
          for(int i = 0; i < string.length(); i++) {
            char c = string.charAt(i);
            // FIXED: Changed condition to only escape control characters, not UTF-8
            if(c <= 31) {  // Only control characters, NOT c >= 127
              if(c == 7) {
                out.print("\\a");
              } else if(c == 8) {
                out.print("\\b");
              ...
              // FIXED: All other characters (including UTF-8) output directly
