...
class LHeaderType53 extends LHeaderType {
  ...
  protected void parse_main(ByteBuffer buffer, BHeader header, LHeaderParseState s) {
    ...
    // Integer size (next byte)
    s.lIntegerSize = 0xFF & buffer.get();
    if (header.debug) System.out.println("-- Lua integer size: " + s.lIntegerSize);
  
    // Peek next byte: if it's 0x78, XLua skipped float size
    buffer.mark();
    int peek = 0xFF & buffer.get();
    if (peek != 0x78) {
      s.lFloatSize = peek;
    } else {
      s.lFloatSize = 8; // default double
      buffer.reset();   // rewind one byte (so endian parser sees 0x78)
    }
    if (header.debug) System.out.println("-- Lua float size: " + s.lFloatSize);
  
    // Read endian block (XLua padded 8 bytes)
    byte[] endianness = new byte[s.lIntegerSize];
    buffer.get(endianness);
  
    boolean little = false, big = false;
    for (int i = 0; i < endianness.length - 1; i++) {
      byte a = endianness[i];
      byte b = endianness[i + 1];
      if (a == (byte) 0x78 && b == (byte) 0x56) { little = true; break; }
      if (a == (byte) 0x56 && b == (byte) 0x78) { big = true; break; }
      if (a == (byte) 0x56 && b == 0x00)        { little = true; break; }
    }
  
    if (little) {
      buffer.order(ByteOrder.LITTLE_ENDIAN);
      if (header.debug) System.out.println("-- XLua endian marker: little-endian");
    } else if (big) {
      buffer.order(ByteOrder.BIG_ENDIAN);
      if (header.debug) System.out.println("-- XLua endian marker: big-endian");
    } else {
      throw new IllegalStateException("Unrecognized endian marker: " + Arrays.toString(endianness));
    }
  
    // Define number types (now guaranteed sane)
    ...
    if (Math.abs(floatcheck - s.lfloat.convert(370.5)) > 1e-3) {
      System.err.println("[WARN] Nonstandard float format: " + floatcheck + " (continuing anyway)");
    }
  }
